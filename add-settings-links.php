...
trait ASL_EnhancedSettingsDetection
{
    ...
    private function fallback_find_settings_url($classOrObject, string $method): array
    {
        ...
        $common_patterns_entry_points = [
            'settings'      => ['admin.php', 'options-general.php', 'settings.php'],
            'options'       => ['admin.php', 'options-general.php', 'settings.php'],
            'configure'     => ['admin.php', 'tools.php'],
            'config'        => ['admin.php', 'tools.php'],
            'setup'         => ['admin.php'],
            'admin'         => ['admin.php'],
            'preferences'   => ['admin.php', 'options-general.php'],
            'prefs'         => ['admin.php', 'options-general.php'],
            'management'    => ['admin.php'],
            'admin-panel'   => ['admin.php'],
            'dashboard'     => ['admin.php'], // Dashboard - less common, but included
        ];
        ...
        if (!empty($plugin_slug_bases)) {
            foreach ($plugin_slug_bases as $plugin_base) {
                if (empty($plugin_base)) continue;

                foreach (static::$settings_terms as $lang => $terms) {
                    foreach ($terms as $term) {
                        $plugin_specific_patterns = [
                            "{$plugin_base}-{$term}",
                            "{$plugin_base}_{$term}",
                        ...
                        foreach ($plugin_specific_patterns as $plugin_pattern) {
                            foreach (['admin.php', 'options-general.php', 'tools.php', 'settings.php'] as $entry_point) { // Re-iterate all entry points for plugin-specific patterns
    ...
    class ASL_AddSettingsLinks
    {
        ...
        public function add_dynamic_plugin_action_links(): void
        {
            ...
            foreach ($all_plugins as $plugin_file => $plugin_data) {
                ...
                }, 20, 2); // Set accepted arguments to 1
            }
        }
        /**
